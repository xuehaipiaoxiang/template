...
* 真前缀/真后缀 （不包含字符串本身）
* 子串 字符串截取连续的一部分（）
* 子序列 原始序列去掉一部分后组成的新序列
*/

//主串S，模式串P
//数组下标都从1开始

next数组长度 对应 等于 模式串P 的长度
next的位置对应模 式串位置相等位置（包含当前位置）的  最长的 相等的真前后缀
故一个字符时，next1[1] = 0
...
for(int i = 2, j = 0; i <= n; ++i)//从2开始
{
    while(j != 0 && p1[i] != p1[j +1]) 匹配上位置的下一个位置
        j = next1[j]; //当前位置已经不等，next数组取匹配上的位置
    if( p1[i] == p1[j + 1] ) ++j;
    next1[ i ] = j;
}
